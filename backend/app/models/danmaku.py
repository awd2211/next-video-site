...
class Danmaku(Base):
    ...
    type = Column(SQLEnum(DanmakuType), default=DanmakuType.SCROLL, nullable=False)  # 弹幕类型
    color = Column(String(7), default="#FFFFFF", nullable=False)  # 颜色 (十六进制)
    font_size = Column(Integer, default=25, nullable=False)  # 字体大小

    # 审核相关
    status = Column(SQLEnum(DanmakuStatus), default=DanmakuStatus.APPROVED, nullable=False, index=True)
