...
from sqlalchemy import Column, Integer, String, DateTime, BigInteger, Float, Boolean, ForeignKey, Text
...
class UploadStatistics(Base):
    ...
    # 上传信息
    upload_id = Column(String(64), nullable=False, index=True, comment="上传会话ID")
    filename = Column(String(255), nullable=False, comment="文件名")
    file_size = Column(BigInteger, nullable=False, comment="文件大小（字节）")
    mime_type = Column(String(100), comment="MIME 类型")
    upload_type = Column(String(50), comment="上传类型 (video/poster/backdrop)")
    ...
    uploaded_chunks = Column(Integer, comment="已上传分片数")
    duration_seconds = Column(Float, comment="上传耗时（秒）")
    upload_speed = Column(Float, comment="平均上传速度（字节/秒）")

    # 存储信息
    object_name = Column(String(512), comment="MinIO 对象名称")
    minio_upload_id = Column(String(255), comment="MinIO multipart upload ID")
    ...
    # IP 地址
    ip_address = Column(String(45), comment="上传者 IP 地址")
