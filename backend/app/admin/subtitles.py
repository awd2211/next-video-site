...
async def create_subtitle(
    ...
    existing_query = select(Subtitle).where(
        and_(Subtitle.video_id == video_id, Subtitle.language == subtitle_data.language)
    )
    existing_result = await db.execute(existing_query)
    existing_subtitle = existing_result.scalar_one_or_none()
    if existing_subtitle:
        raise HTTPException(
            status_code=400, detail=f"该视频已存在 {subtitle_data.language_name} 字幕"
        )
...
async def update_subtitle(
    ...
    if subtitle_data.is_default is True:
        await db.execute(
            update(Subtitle)
            .where(
                and_(Subtitle.video_id == subtitle.video_id, Subtitle.id != subtitle_id)
            )
            .values(is_default=False)
        )
...
async def delete_subtitle(
    ...
    try:
        ...
        logger.info(
            f"✅ MinIO字幕文件已删除: video_{subtitle.video_id}_{subtitle.language}.{subtitle.format}"
        )
...
async def upload_subtitle_file(
    ...
    if file_ext not in ["srt", "vtt", "ass"]:
        raise HTTPException(
            status_code=400, detail="不支持的字幕格式,仅支持 .srt, .vtt, .ass"
        )
    ...
    if file_ext == "srt":
        try:
            ...
            # Fallback: 上传原始SRT
            file_url = minio_client.upload_subtitle(
                io.BytesIO(content), video_id=video_id, language=language, format="srt"
            )
    else:
        # 直接上传VTT或其他格式
        file_url = minio_client.upload_subtitle(
            io.BytesIO(content), video_id=video_id, language=language, format=file_ext
        )
