...
class CacheWarmer:
    ...
    async def warm_countries():
        """预热国家数据"""
        logger.info("Warming up countries cache...")
        async with async_session_maker() as db:
            ...
            logger.info(f"Cached {len(countries)} countries")
    ...
    async def warm_tags():
        ...
        async with async_session_maker() as db:
            ...
            logger.info(f"Cached {len(tags)} tags")

    @staticmethod
    async def warm_trending_videos(page_size: int = 20):
        """预热热门视频数据"""
        logger.info("Warming up trending videos cache...")
        async with async_session_maker() as db:
            # 预热前3页
            for page in range(1, 4):
                ...
                logger.info(f"Cached trending videos page {page}")

    @staticmethod
    async def warm_featured_videos(page_size: int = 20):
        """预热推荐视频数据"""
        logger.info("Warming up featured videos cache...")
        async with async_session_maker() as db:
            # 预热前2页
            for page in range(1, 3):
                ...
                logger.info(f"Cached featured videos page {page}")

    @staticmethod
    async def warm_recommended_videos(page_size: int = 20):
        """预热推荐视频数据"""
        logger.info("Warming up recommended videos cache...")
        async with async_session_maker() as db:
            # 预热前2页
            for page in range(1, 3):
                query = (
                ...
                await Cache.set(cache_key, response, ttl=900)
                logger.info(f"Cached recommended videos page {page}")
