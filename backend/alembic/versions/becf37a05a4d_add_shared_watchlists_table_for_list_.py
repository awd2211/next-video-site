"""Add shared_watchlists table for list sharing feature

Revision ID: becf37a05a4d
Revises: 3e4ccd73dc9f
Create Date: 2025-10-14 00:07:52.124407
...
revision: str = 'becf37a05a4d'
down_revision: Union[str, None] = '3e4ccd73dc9f'
...
def upgrade() -> None:
    ...
    # Create shared_watchlists table
    op.create_table('shared_watchlists',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('user_id', sa.Integer(), nullable=False),
    sa.Column('share_token', sa.String(length=32), nullable=False, comment='Unique token for sharing'),
    sa.Column('title', sa.String(length=200), nullable=False, comment='Custom title for shared list'),
    sa.Column('description', sa.Text(), nullable=True, comment='Optional description'),
    sa.Column('video_ids', sa.Text(), nullable=False, comment='Comma-separated video IDs'),
    sa.Column('is_active', sa.Boolean(), nullable=False, server_default=sa.text('true'), comment='Whether the share link is active'),
    sa.Column('view_count', sa.Integer(), nullable=False, server_default=sa.text('0'), comment='Number of views'),
    ...
    if not table_exists:
        ...
        result = conn.execute(sa.text("""
            SELECT EXISTS (
                SELECT FROM pg_type
                WHERE typname = 'shareplatform'
            )
        """))
        enum_exists = result.scalar()

        if not enum_exists:
            op.execute("CREATE TYPE shareplatform AS ENUM ('WECHAT', 'WEIBO', 'QQ', 'QZONE', 'TWITTER', 'FACEBOOK', 'LINK', 'OTHER')")

        op.create_table('video_shares',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('video_id', sa.Integer(), nullable=False),
        sa.Column('user_id', sa.Integer(), nullable=True),
        sa.Column('platform', sa.Enum('WECHAT', 'WEIBO', 'QQ', 'QZONE', 'TWITTER', 'FACEBOOK', 'LINK', 'OTHER', name='shareplatform', create_type=False), nullable=False),
