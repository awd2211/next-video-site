...
describe('File Validation Utils', () => {
  ...
  describe('validateFile', () => {
    it('should accept valid image file', () => {
      ...
      const result = validateFile(file, FileValidationPresets.poster)
      expect(result.valid).toBe(true)
    })

    it('should reject file that is too large', () => {
      const file = createMockFile('large.jpg', 'image/jpeg', 20 * 1024 * 1024) // 20MB
      const result = validateFile(file, FileValidationPresets.poster) // max 10MB
  ...
  describe('validateFiles', () => {
    it('should validate multiple files', () => {
      ...
      const result = validateFiles(files, FileValidationPresets.poster)
      expect(result.valid).toBe(true)
  ...
  describe('FileValidationPresets', () => {
    it('should have poster preset', () => {
      expect(FileValidationPresets.poster).toBeDefined()
      expect(FileValidationPresets.poster.maxSize).toBe(10)
      expect(FileValidationPresets.poster.allowedTypes).toContain('image/jpeg')
    })
    ...
    it('should have thumbnail preset', () => {
      expect(FileValidationPresets.thumbnail).toBeDefined()
      expect(FileValidationPresets.thumbnail.maxSize).toBe(5)
    })
  })
})
