...
#### 1. **AV1转码集成到上传流程** ❌
**当前状态**: AV1工具类已实现,但未集成到视频上传API
**缺失内容**:
```python
# backend/app/api/videos.py - 需要修改上传端点
@router.post("/upload")
async def upload_video(...):
    # 1. 保存原始视频
    # 2. 触发H.264转码 (快速上线)
    # 3. 🔴 缺失: 触发AV1转码任务
    transcode_video_dual_format.delay(video.id)  # ← 需要添加
```

**影响**: 用户上传的视频无法自动获得AV1优化
**工作量**: 1-2小时

---

#### 2. **视频播放进度保存** ⚠️
**当前状态**: WatchHistory模型存在,但缺少进度更新API
**缺失内容**:
```python
# backend/app/api/history.py - 缺少进度更新端点
@router.patch("/watch-history/{video_id}/progress")
...
**前端需求**:
```typescript
// frontend/src/components/VideoPlayer/ - 缺少进度同步
useEffect(() => {
  const interval = setInterval(() => {
    // 每10秒保存一次进度
    api.updateWatchProgress(videoId, player.currentTime());
  }, 10000);
}, []);
```
...
#### 3. **字幕上传和管理** ❌
**当前状态**: 有文档 ([subtitle-management.md](../features/subtitle-management.md)),但无实现
**缺失内容**:

**后端**:
```python
# backend/app/models/subtitle.py - 需要创建
class Subtitle(Base):
    id = Column(Integer, primary_key=True)
    video_id = Column(Integer, ForeignKey('videos.id'))
    language = Column(String(10))  # zh-CN, en-US
    format = Column(String(10))    # SRT, VTT, ASS
    url = Column(Text)
    is_default = Column(Boolean, default=False)

# backend/app/api/subtitles.py - 需要创建
