...
import { directorService, DirectorDetail } from '../../services/directorService'
...
const DirectorDetailPage = () => {
  ...
  useEffect(() => {
    const loadDirectorData = async () => {
      ...
      try {
        setLoading(true)
        const directorData = await directorService.getDirector(Number(id))
        setDirector(directorData)

        const videosData = await directorService.getDirectorVideos(Number(id), videosPage, pageSize)
  ...
  if (!director) {
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center">Director not found</div>
      </div>
    )
  }
    ...
    <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row">
          <div className="flex-shrink-0 mb-4 md:mb-0">
            {director.avatar ? (
              ...
              <div className="w-48 h-48 rounded-lg bg-gray-300 flex items-center justify-center">
                <span className="text-6xl text-gray-600">{director.name[0]}</span>
              </div>
            )}
          </div>
          ...
          <div className="md:ml-6 flex-grow">
            ...
            <div className="space-y-2 text-gray-700">
              {director.birth_date && (
                <div>
                  <span className="font-semibold">Born:</span> {formatDate(director.birth_date)}
                </div>
              )}
            </div>
            ...
            {director.biography && (
              <div className="mt-4">
                ...
                <p className="text-gray-700 whitespace-pre-line">{director.biography}</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div>
        <h2 className="text-2xl font-bold mb-4">
          Directed ({videos?.total || 0} {videos?.total === 1 ? 'title' : 'titles'})
        </h2>
        ...
        {videos && videos.items && videos.items.length > 0 ? (
          ...
          <div className="text-center py-8 text-gray-500">
            No videos found for this director.
          </div>
        )}
      </div>
    </div>
